...
# Fecth user role id
user_rid=$(./get_role_id.py "$(curl -X GET -H "Accept: application/json" -H "Content-Type: application/json" -H "Authorization: bearer $token" -s "${KEYCLOAK_BASE_URL}/admin/realms/${REALM}/roles" && echo)" "bigoptibase_user")
echo "user_rid : $user_rid"

# map admin user to user role 
curl -ik -X POST -H "Accept: application/json" -H "Content-Type: application/json" -H "Authorization: bearer $token" -d "[{\"id\": \"${user_rid}\", \"name\": \"bigoptibase_user\"}]" -s "${KEYCLOAK_BASE_URL}/admin/realms/${REALM}/users/${uid}/role-mappings/realm" && echo
