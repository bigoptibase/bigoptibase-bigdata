...
do
    ...
    PODS=$(kubectl get pods -n bigoptibase -o wide | grep 'bda-' | awk '{print $1}')
    IPS=$(kubectl get pods -n bigoptibase -o wide | grep 'bda-' | awk '{print $6 " " $8}' | sed -E 's/<none>+\ ?//g' | sed -E 's/ago\)+\ ?//g')
    ...
    echo "IPs are not the same. Updating hostnames..."
        ...
        for pod in $PODS; do
            ...
            kubectl exec -it $pod -n bigoptibase -- sh -c "echo '\n\n\n\n\n' >> /etc/hosts"
            kubectl exec -it $pod -n bigoptibase -- sh -c "head -n 15 /etc/hosts > /etc/tempfile && cat /etc/tempfile > /etc/hosts"
            ...
            for other_pod in $PODS; do
                ...
                then
                    ...
                    kubectl exec -i $pod -n bigoptibase -- sh -c "echo '$cur_ip $cur_pod_hostname' >> /etc/hosts"
                    j=$((j+1))
