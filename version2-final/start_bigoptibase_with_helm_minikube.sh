...
kubectl create namespace bigoptibase

###### REMOVE OLD DEPLOYMENTS ON KUBERNETES CLUSTER
helm uninstall network-observer -n bigoptibase #--purge
helm uninstall metrics-server
helm uninstall zookeeper -n bigoptibase
helm uninstall hadoop-namenode -n bigoptibase
helm uninstall hadoop-journalnode -n bigoptibase
helm uninstall hadoop-datanode -n bigoptibase
helm uninstall hadoop-yarn-rm -n bigoptibase
helm uninstall spark -n bigoptibase
helm uninstall hbase-master -n bigoptibase
helm uninstall hbase-regionserver -n bigoptibase
helm uninstall livy -n bigoptibase
helm uninstall postgres -n bigoptibase
#sleep 15
if [ "$(kubectl get pods -n bigoptibase | grep pgo)" != "" ]
then
    while [ "$(kubectl get pods -n bigoptibase | grep postgres)" != "" ]; do 
        sleep 5; 
        echo "Waiting for Postgres Chart to shut down.";
    done
    helm uninstall pgo -n bigoptibase # if postgres HA is enabled
fi
helm uninstall kafka -n bigoptibase
helm uninstall keycloak -n bigoptibase
helm uninstall bda -n bigoptibase

kubectl delete pvc --all -n bigoptibase # Helm will not destroy PVCs since they are not created/controlled by Helm
...
# HA ZK enabled
    ...
    for i in $(seq 1 $ZKreplicas); do
        echo "server.${i}=bda-zookeeper-master-$((${i}-1)).bda-zookeeper-master.bigoptibase.svc.cluster.local:2888:3888" >> docker/zookeeper/zoo.cfg
    done
fi
...
# HA NN enabled
    ...
    for i in $(seq 1 $HDFS_NNreplicas); do 
        HDFS_SITE_CONTENT="${HDFS_SITE_CONTENT}\n\t<property>\n\t\t<name>dfs.namenode.rpc-address.bda-hadoop-namenode.nn${i}</name>\n\t\t<value>bda-hadoop-namenode-$((${i}-1)).bda-hadoop-namenode.bigoptibase.svc.cluster.local:8020</value>\n\t</property>"
        HDFS_SITE_CONTENT="${HDFS_SITE_CONTENT}\n\t<property>\n\t\t<name>dfs.namenode.http-address.bda-hadoop-namenode.nn${i}</name>\n\t\t<value>bda-hadoop-namenode-$((${i}-1)).bda-hadoop-namenode.bigoptibase.svc.cluster.local:50070</value>\n\t</property>"
    done


    zk_addr="" 
    for i in $(seq 1 $ZKreplicas); do
        zk_addr+="bda-zookeeper-master-$((${i}-1)).bda-zookeeper-master.bigoptibase.svc.cluster.local:2181,"
    done
    zk_addr=${zk_addr%?}
    ...
    for i in $(seq 1 $HDFS_JNreplicas); do
        jn_addr+="bda-hadoop-journalnode-$((${i}-1)).bda-hadoop-journalnode.bigoptibase.svc.cluster.local:8485;"
    done
    jn_addr=${jn_addr%?}
    # for journalnodes
...
if [[ $ZKreplicas == 1 ]] ; then
    ...
    zk_addr=""
    for i in $(seq 1 $ZKreplicas); do     
        zk_addr+="bda-zookeeper-master-$((${i}-1)).bda-zookeeper-master.bigoptibase.svc.cluster.local,"
    done
    zk_addr=${zk_addr%?}
    echo '
...
if [[ $HDFS_NNreplicas > 1 ]] ; then
    ...
    nnAvailable=""
    for i in $(seq 1 $HDFS_NNreplicas); do
        nnAvailable+="hdfs://bda-hadoop-namenode-$((${i}-1)).bda-hadoop-namenode.bigoptibase.svc.cluster.local,"
    done
...
# HA POSTGRES enabled
    ...
    sed -i "s/bda-postgres:/bda-postgres-primary.bigoptibase.svc.cluster.local:/g" conf/bda.properties    
else
    echo 'POSTGRES HA not enabled'
    # update url of unique postgres instance in bda.properties
    sed -i "s/bda-postgres-primary.bigoptibase.svc.cluster.local:/bda-postgres:/g" conf/bda.properties    
fi
...
docker build \
    --tag bigoptibase/bda-network-observer:latest \
...
if [[ $HDFS_NNreplicas > 1 ]] ; then
    ...
    docker build \
        ...
        --tag bigoptibase/hadoop-journalnode-$HADOOP_VERSION \
...
helm install network-observer kubernetes/helm/network-observer -n bigoptibase

#Start metrics-server for HPA to work
#helm install metrics-server kubernetes/helm/metrics-server
helm install zookeeper kubernetes/helm/zookeeper -n bigoptibase
#sleep 5

if [[ $HDFS_NNreplicas > 1 ]] ; then
    helm install hadoop-journalnode kubernetes/helm/hadoop-journalnode -n bigoptibase
    sleep 10
fi
helm install bda kubernetes/helm/bda -n bigoptibase
helm install hadoop-namenode kubernetes/helm/hadoop-namenode -n bigoptibase
sleep 10
helm install hadoop-datanode kubernetes/helm/hadoop-datanode -n bigoptibase
sleep 5
helm install hadoop-yarn-rm kubernetes/helm/hadoop-yarn-rm -n bigoptibase
sleep 5
helm install spark kubernetes/helm/spark -n bigoptibase
helm install hbase-master kubernetes/helm/hbase-master -n bigoptibase
sleep 5
helm install hbase-regionserver kubernetes/helm/hbase-regionserver -n bigoptibase
sleep 5
helm install livy kubernetes/helm/livy -n bigoptibase

if [[ $POSTGRESreplicas > 1 ]] ; then
    helm install pgo kubernetes/helm/postgres-operator/pgo -n bigoptibase
    # Create configmap to execute sql when bootstraping postgresql db
    kubectl -n bigoptibase create configmap bootstrap-sql --from-file=init.sql=./docker/postgres/bootstrap-postgres.sql
    sleep 15
    helm install postgres kubernetes/helm/postgres-operator/postgres -n bigoptibase
else
    helm install postgres kubernetes/helm/postgres -n bigoptibase
fi
sleep 10
helm install keycloak kubernetes/helm/keycloak -n bigoptibase
sleep 25
helm install kafka kubernetes/helm/kafka -n bigoptibase
